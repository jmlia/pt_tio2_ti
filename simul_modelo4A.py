...
Script para generar la curva IV simulando el circuito del modelo 4A (dos diodos
...
usada en el script fit_modelo4A.
...
def generar_I(V, A, n, R, I_0, b, start):
    """
    ...
    I_sch = lambda I: I_0 * ( np.exp(b * np.sqrt(V)) - 1)
    
    
    implicit = lambda I: I - I_sclc - I_r - I_sch(I)
...
def generar_IV(V, A, n, R, I_0, b): 
    ...
    I = np.zeros(len(V))
    I[0] = generar_I(V[0], A, n, R, I_0, b, 0)

    for i in range(len(V)):
    
        I[i] = generar_I(V[i], A, n, R, I_0, b, I[i-1])
